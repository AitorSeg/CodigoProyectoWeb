...
window.obtenerExamenesAsignatura = function (idAsignatura) {
    return window.DOA_EXAMENES[idAsignatura] || window.DOA_EXAMENES.matematicas;
};

window.guardarExamenSeleccionado = function (idExamen) {
...
window.buscarExamenPorId = function (idExamen) {
...
window.obtenerExamenActual = function () {
    const idExamen = window.obtenerExamenSeleccionado();
    const examenGuardado = window.buscarExamenPorId(idExamen);
    ...
    const examenesAsignatura = window.obtenerExamenesAsignatura(idAsignatura);

    return examenesAsignatura[0];
};